...
export function StoreCurrency({ currency, currentCurrency }: { currency: GetStoreConfigQuery['currency'], currentCurrency?: string }) {
    const [mounted, setMounted] = useState(false)

    const activeCurrencyCode = currentCurrency || currency?.default_display_currency_code
    ...
    if (!mounted) {
        return (
            <div className="h-9 px-3 py-2 text-sm flex items-center gap-2">
                {currencyIcons[activeCurrencyCode as keyof typeof currencyIcons]} {activeCurrencyCode}
            </div>
        )
    }
